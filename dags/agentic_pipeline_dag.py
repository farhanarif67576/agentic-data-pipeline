...
class Config:
    BASE_DIR = os.getenv('PIPELINE_BASE_DIR', '/workspaces/Hello-world')
...
def _analyze_with_ollama(healed_reviews: list[dict], model_info: dict) -> list[dict]:
    ...
    try:
        ...
        return _create_degraded_results(healed_reviews, str(e))
...
def _create_degraded_results(healed_reviews: list[dict], error_message: str) -> list[dict]:
    return [
        {
            **review,
            'text': review.get('healed_text', ''),
